...
namespace S1FObjectDefinitions.MZ
{
	class SwingPlat : ObjectDefinition
	{
		...
		public override void Init(ObjectData data)
		{
			...
			if (LevelData.StageInfo.folder.EndsWith("BossRush")) // not sure if we should keep using EndsWith since Forever is based off of pre-Origins? may as well keep it like this anyways though, in case they update it
			{
				...
				sprites[0] = new Sprite(sheet.GetSection(76, 330, 16, 16), -8, -8);
				sprites[1] = new Sprite(sheet.GetSection(93, 330, 16, 16), -8, -8);
				sprites[2] = new Sprite(sheet.GetSection(170, 310, 48, 16), -24, -8);
				
				// currently the game does a weird thing where it makes all even length platforms flipped, but i'm assuming they're going to change that in the next update-
			}
			else
			{
				BitmapBits sheet = LevelData.GetSpriteSheet("MZ/Objects.gif");
				sprites[0] = new Sprite(sheet.GetSection(76, 160, 16, 16), -8, -8);
				sprites[1] = new Sprite(sheet.GetSection(93, 160, 16, 16), -8, -8);
				sprites[2] = new Sprite(sheet.GetSection(69, 177, 48, 16), -24, -8);
			}
